  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(ellipse_at_top_left,rgba(16,185,129,0.10),transparent_45%)] px-5 pb-24 pt-8 text-white sm:px-8">
      <div className="mx-auto max-w-6xl">
        ...
        {study.gallery ? (
          <section id="visuals" className="border-t border-white/10 py-12">
            <p className="text-xs uppercase tracking-widest text-emerald-400">Launch / Product / Culture</p>
            <h2 className="mt-3 text-3xl font-semibold">Dripy social media gallery</h2>
            <p className="mt-3 text-sm leading-relaxed text-gray-400">11 creatives spanning pre-launch teasers, product visuals and humour-led campaigns. Select any image to open the full design.</p>
            <div className="mt-8 columns-1 gap-5 sm:columns-2 lg:columns-3">
              {study.gallery.map((item) => (
                <figure key={item.src} className="mb-5 break-inside-avoid overflow-hidden rounded-2xl border border-white/10 bg-black">
                  <a href={item.src} target="_blank" rel="noopener noreferrer" aria-label={`Open full design: ${item.caption}`} className="block focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-emerald-400">
                    <Image src={item.src} alt={item.alt} width={item.width} height={item.height} sizes="(max-width: 640px) 90vw, (max-width: 1024px) 45vw, 360px" className="h-auto w-full" />
                    <figcaption className="flex items-start justify-between gap-3 p-4 text-sm text-gray-300"><span>{item.caption}</span><span aria-hidden="true" className="text-emerald-300"><ArrowUpRight aria-hidden="true" className="inline-block h-5 w-5 align-middle" /></span></figcaption>
                  </a>
                </figure>
              ))}
            </div>
